...
@section('content')
    ...
    {{--/add modal--}}
    {{--edit modal for Candidate--}}
    <div class="modal" id="editModal" tabindex="-1" role="dialog" aria-labelledby="Survey">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center">
                    <h4 class="modal-title" id="exampleModalLabel1">Edit Candidate</h4>
                    <button type="button" class="close ml-auto" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <form  class="form-horizontal"  action="{{route('admin.updateCandidate')}}" id="editForm">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        ...
                        <div class="edit-result">
                            ...
                            <div class="form-group row">
                                <label for="edit-name" class="control-label col-sm-3">Candidate Name</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                            ...
                            <div class="form-group row">
                                <label for="edit-identity" class="control-label col-sm-3">Candidate Indentity</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="edit-identity" name="identity" required autofocus>
                                    <span class="text-danger" id="eperiod" style="color: red"></span>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="edit-name" class="control-label col-sm-3">Candidate party</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="edit-party" name="party" required autofocus>
                                    <span class="text-danger" id="eperiod" style="color: red"></span>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="edit-name" class="control-label col-sm-3">Candidate Strength</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="edit-strength" name="strength" required autofocus>
                                    <span class="text-danger" id="eperiod" style="color: red"></span>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="edit-date1" class="control-label col-sm-3">Candidate dob</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                ...
                columns: [
                    ...
                    {data: 'party'},
                    {
                        ...
                        render: function (data, type, row) {
                            return"<button  data-url='/home/candidate/show/" + row.id + "' class='btn btn-info btn-sm btn-flat js-edit' data-id='" + data +
                                "' > <i class='fa fa-edit'></i>Edit</button>" +
                                "<button class='btn btn-danger btn-sm btn-flat js-delete ' data-id='" + data +
                                "' data-url='/home/candidate/delete/" + row.id + "'> <i class='fa fa-trash'></i>Delete</button>";
            ...
            manageTable.on('click', '.js-edit', function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $("#edit-name").val(response.candidate.name);
                        $("#edit-identity").val(response.candidate.identity);
                        $("#edit-party").val(response.candidate.party);
                        $("#edit-strength").val(response.candidate.strength);
                        $("#edit-dob").val(response.candidate.dob);

                        // add hidden id
                        $('#id').val(response.candidate.id);
                        // update - form
...
//                                form[0].reset();
                                ...
                                $('#edit-messages').html('<div class="alert alert-success">' +
                                    '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Candidate  successfully updated. </div>');
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                bootbox.alert({
                                    title: "Error",
                                    message: "<i class='fa fa-warning'></i>" +
                                        " Candidate not Delete please try again"
                                });
                            }
                        });
