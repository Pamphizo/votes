        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center">
                    <h4 class="modal-title" id="exampleModalLabel1">Add New Province</h4>
                    <button type="button" class="close ml-auto" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <form data-parsley-validate class="form-horizontal" method="POST" action="{{route('admin.saveProvince')}}" id="frmSave">
                    {{ csrf_field() }}
                    ...
                    <div class="modal-footer">
                        ...
                        <input type="submit" class="btn btn-primary" id="btnSave" data-loading-text="Loading..." value="Save Province"/>
                    </div>
                </form>
            </div>
        </div>
...
@section('js')
    <script>
        ...
        var defaultUrl = "{{ route('admin.getProvince') }}";
        var table;
                ...
                columns: [
                    ...
                    {data: 'name'},
                    {
                        ...
                        render: function (data, type, row) {
                                return"<button  data-url='/home/province/show/" + row.id + "' class='btn btn-info btn-sm btn-flat js-edit' data-id='" + data +
                                    "' > <i class='fa fa-edit'></i>Edit</button>" +
                                    "<button class='btn btn-danger btn-sm btn-flat js-delete ' data-id='" + data +
                                    "' data-url='/home/province/delete/" + row.id + "'> <i class='fa fa-trash'></i>Delete</button>";
                ...
                $.ajax({
                    ...
                    if (data.province == "ok") {
                        ...
                        $('#add-messages').html('<div class="alert alert-success flat">' +
                            ...
                            '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Province  successfully saved. </div>');
            ...
            manageTable.on('click', '.js-edit', function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $("#edit-name").val(response.province.name);

                        // add hidden id
                        $('#id').val(response.province.id);
                        // update - form
...
//                                form[0].reset();
                                ...
                                $('#edit-messages').html('<div class="alert alert-success">' +
                                    '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Province  successfully updated. </div>');
            ...
            manageTable.on('click', '.js-delete', function () {
                var button = $(this);
                bootbox.confirm("Are you sure you want to Delete this Province?", function (result) {
                    if (result) {
                        $.ajax({
                           ...
                           var tr = button.parents("tr");
                                ...
                                bootbox.alert({
                                    title: "Error",
                                    message: "<i class='fa fa-warning'></i>" +
                                        " Province not Delete please try again"
                                });
                            }
                        });
